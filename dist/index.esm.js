...
class ClaudeProvider {
    constructor(param, defaultModel, options) {
        ...
        Please refer to the link: https://lyvo.fellou.ai/docs/getting-started/configuration#web-environment
      `);
        }
...
class WorkflowImpl {
    constructor(id, name, lyvoConfig, rawWorkflow, description, nodes = [], variables = new Map(), llmProvider, loggerOptions) {
        this.id = id;
        this.name = name;
...
class ActionImpl {
                ...
                onToolUse: async (toolCall) => {
                    ...
                    toolExecutionPromise = (async () => {
                        ...
                        try {
                            ...
                            // afterToolUse
                            let existingTabs = await context.lyvoConfig.chromeProxy.tabs.query({});
    ...
    async execute(input, output, context, outputSchema) {
        ...
        // get patchs for task
        let patchs = [];
        if (context.lyvoConfig.patchServerUrl) {
            patchs = await this.getPatchs(this.name, context.lyvoConfig.patchServerUrl);
        }
...
function createWorkflowPrompts(tools) {
    return {
        formatSystemPrompt: () => {
            return `You are a workflow generation assistant that creates Lyvo framework workflows.
...
function createWorkflowGenerationTool(registry) {
    return {
        ...
        description: `Generate a workflow following the Lyvo framework DSL schema.
...
class WorkflowGenerator {
    ...
    async generateWorkflow(prompt, lyvoConfig) {
        return this.doGenerateWorkflow(prompt, false, lyvoConfig);
    }
    async generateWorkflowFromJson(json, lyvoConfig) {
        return this.createWorkflowFromData(json, lyvoConfig);
    }
    async modifyWorkflow(prompt, lyvoConfig) {
        return this.doGenerateWorkflow(prompt, true, lyvoConfig);
    }
    async doGenerateWorkflow(prompt, modify, lyvoConfig) {
        ...
        while (retry_counter > 0) {
            try {
                ...
                return this.createFastWorkflowFromData(workflowData, lyvoConfig);
            }
            catch (e) {
    ...
    createWorkflowFromData(data, lyvoConfig) {
        const workflow = new WorkflowImpl(data.id, data.name, lyvoConfig, data, data.description || '', [], new Map(Object.entries(data.variables || {})), this.llmProvider, {
            logLevel: 'info',
    ...
    createFastWorkflowFromData(data, lyvoConfig) {
        const workflow = new WorkflowImpl(data.id, data.name, lyvoConfig, data, data.description || '', [], new Map(Object.entries(data.variables || {})), this.llmProvider, {
            logLevel: 'info',
            includeTimestamp: true,
        });
 ...
 */
class Lyvo {
    constructor(llmConfig, lyvoConfig) {
        ...
        this.registerTools();
        logger.info("using Lyvo@" + "d6e5f731cf4ef8a147d77258296b48933254d207");
...
class WorkflowParser {
    ...
    static parse(json, lyvoConfig) {
        ...
        return this.toRuntime(parsed, lyvoConfig);
    }
    /**
    ...
    static toRuntime(json, lyvoConfig) {
        const variables = new Map(Object.entries(json.variables || {}));
        const workflow = new WorkflowImpl(json.id, json.name, lyvoConfig, json, json.description, [], variables, undefined, {
            logLevel: 'info',
            includeTimestamp: true,
        });
        // Convert nodes
...
export { ClaudeProvider, Lyvo, ExecutionLogger, LLMProviderFactory, OpenaiProvider, ToolRegistry, WorkflowGenerator, WorkflowParser, createChromeApiProxy, Lyvo as default };
