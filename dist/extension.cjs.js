...
async function getWindowId(context) {
    ...
    if (!windowId) {
        windowId = context.lyvoConfig.workingWindowId;
    }
    if (!windowId) {
...
async function getTabId(context) {
    ...
    let tabs = await context.lyvoConfig.chromeProxy.tabs.query({});
    ...
    if (filtered.length > 0) {
        if (typeof filtered[0].activeTime != "undefined") {
            ...
            tabs = await context.lyvoConfig.chromeProxy.tabs.query({ active: true, currentWindow: true });
            if (tabs.length > 0) {
        ...
        try {
            ...
            switch (params.action) {
                case 'input_text':
                    ...
                    await clear_input_by(context.lyvoConfig.chromeProxy, tabId, selector_xpath, params.index);
                    result = await type_by(context.lyvoConfig.chromeProxy, tabId, params.text, selector_xpath, params.index);
                    await sleep(200);
                    break;
                case 'click':
                    if (params.index == null) {
                    ...
                    result = await left_click_by(context.lyvoConfig.chromeProxy, tabId, selector_xpath, params.index);
                    await sleep(100);
                    break;
                case 'right_click':
                    if (params.index == null) {
                    ...
                    result = await right_click_by(context.lyvoConfig.chromeProxy, tabId, selector_xpath, params.index);
                    await sleep(100);
                    break;
                case 'double_click':
                    if (params.index == null) {
                    ...
                    result = await double_click_by(context.lyvoConfig.chromeProxy, tabId, selector_xpath, params.index);
                    await sleep(100);
                    break;
                case 'scroll_to':
                    if (params.index == null) {
                    ...
                    result = await scroll_to_by(context.lyvoConfig.chromeProxy, tabId, selector_xpath, params.index);
                    await sleep(500);
                    break;
                ...
                case 'get_dropdown_options':
                    ...
                    result = await get_dropdown_options(context.lyvoConfig.chromeProxy, tabId, selector_xpath, params.index);
                    break;
                case 'select_dropdown_option':
                    ...
                    result = await select_dropdown_option(context.lyvoConfig.chromeProxy, tabId, params.text, selector_xpath, params.index);
                    break;
                case 'screenshot_extract_element':
                    logger.debug("execute 'screenshot_extract_element'...");
                    await sleep(100);
                    logger.debug("injectScript...");
                    await injectScript(context.lyvoConfig.chromeProxy, tabId, 'build_dom_tree.js');
                    await sleep(100);
                    try {
                        logger.debug("executeScript...");
                        let element_result = await executeScript(context.lyvoConfig.chromeProxy, tabId, () => {
                            return window.get_clickable_elements(true);
                        }, []);
                        context.selector_map = element_result.selector_map;
                        logger.debug("browser.screenshot...");
                        let screenshot$1 = await screenshot(context.lyvoConfig.chromeProxy, windowId, true);
                        result = { image: screenshot$1.image, text: element_result.element_str };
                    }
                    finally {
                        await sleep(500);
                        logger.debug("executeScript #2...");
                        await executeScript(context.lyvoConfig.chromeProxy, tabId, () => {
...
class ExportFile {
    ...
    async execute(context, params) {
        ...
        try {
            ...
            await context.lyvoConfig.chromeProxy.scripting.executeScript({
                target: { tabId: tabId },
                func: exportFile,
            ...
            await sleep(5000);
            await context.lyvoConfig.chromeProxy.tabs.remove(tabId);
        }
        return { success: true };
    }
}
...
class OpenUrl extends ToolReturnsScreenshot {
    ...
    async realExecute(context, params) {
        ...
        if (newWindow) {
            ...
            let windowId = context.lyvoConfig.workingWindowId ? context.lyvoConfig.workingWindowId : await getWindowId(context);
            logger.debug('Using existing window with ID:', windowId);
            try {
                tab = await open_new_tab(context.lyvoConfig.chromeProxy, url, windowId);
...
class Screenshot {
    ...
    async execute(context, params) {
        let windowId = await getWindowId(context);
        return await screenshot(context.lyvoConfig.chromeProxy, windowId);
    }
}
...
* \`switch_tab\`: Switch to the specified tab by tabId.`,
    ...
    async execute(context, params) {
        ...
        if (params.command == 'tab_all') ;
            ...
            let tabId = await getTabId(context);
            await context.lyvoConfig.chromeProxy.tabs.goBack(tabId);
        }
        else if (params.command == "switch_tab") {
            await context.lyvoConfig.chromeProxy.tabs.select(params.tabId);
        }
        else if (params.command == 'close_tab') {
            let closedTabId = await getTabId(context);
            await context.lyvoConfig.chromeProxy.tabs.remove(closedTabId);
            await sleep(100);
            let tabs = await context.lyvoConfig.chromeProxy.tabs.query({ active: true, currentWindow: true });
            if (tabs.length == 0) {
                tabs = await context.lyvoConfig.chromeProxy.tabs.query({ status: 'complete', currentWindow: true });
            }
            let tab = tabs[tabs.length - 1];
            if (!tab.active) {
                await context.lyvoConfig.chromeProxy.tabs.update(tab.id, { active: true });
            }
        ...
        if (tabs.length > 0) {
            ...
            let currentTab = await context.lyvoConfig.chromeProxy.tabs.get(currentTabId);
    ...
    destroy(context) {
        ...
        if (windowIds) {
            for (let i = 0; i < windowIds.length; i++) {
                context.lyvoConfig.chromeProxy.windows.remove(windowIds[i]);
            }
        }
    }
}
...
class WebSearch {
    ...
    async execute(context, params) {
        ...
        let searchInfo = await deepSearch(context, taskId, searchs, maxResults || 5, context.lyvoConfig.workingWindowId);
...
// TODO: replace `chrome` with `context.lyvoConfig.chromeProxy`
...
async function deepSearch(context, taskId, searchs, detailsMaxNum, windowId) {
    ...
    if (!windowId) {
        // open new window
        let window = await context.lyvoConfig.chromeProxy.windows.create({
            type: 'normal',
            state: 'maximized',
            url: null,
        });
    ...
    // close window
    closeWindow && context.lyvoConfig.chromeProxy.windows.remove(windowId);
    return searchInfo;
}
/**
 * crawler the search page details page link
...
async function doDetailLinkGroups(context, taskId, searchs, detailsMaxNum, windowId) {
    ...
    for (let i = 0; i < searchs.length; i++) {
        try {
            ...
            // open new Tab
            let tab = await context.lyvoConfig.chromeProxy.tabs.create({
                url: url,
                windowId,
            });
            (_c = (_b = (_a = context.callback) === null || _a === void 0 ? void 0 : _a.hooks) === null || _b === void 0 ? void 0 : _b.onTabCreated) === null || _c === void 0 ? void 0 : _c.call(_b, tab.id);
            ...
            tabsUpdateEvent.addListener(async function (obj) {
                ...
                if (obj.changeInfo.status === 'complete') {
                    ...
                    // inject js
                    await injectScript(context.lyvoConfig.chromeProxy, tab.id, filename);
                    await sleep(1000);
                    // crawler the search page details page
                    // { links: [{ title, url }] }
                    let detailLinks = await context.lyvoConfig.chromeProxy.tabs.sendMessage(tab.id, {
                    ...
                    context.lyvoConfig.chromeProxy.tabs.remove(tab.id);
                }
    ...
    for (let i = 0; i < detailLinkGroups.length; i++) {
        ...
        for (let j = 0; j < links.length; j++) {
                ...
                tabsUpdateEvent.addListener(async function onTabUpdated(obj) {
                    ...
                    if (obj.changeInfo.status === 'complete') {
                        ...
                        try {
                            ...
                            await injectScript(context.lyvoConfig.chromeProxy, tab.id, filename);
                            await sleep(1000);
                            let result = await context.lyvoConfig.chromeProxy.tabs.sendMessage(tab.id, {
            ...
            try {
                ...
                searchInfo.failedLinks.push(link);
                countDownLatch.countDown();
                context.lyvoConfig.chromeProxy.tabs.remove(tab.id); // Clean up tab on failure
            }
        }
    }
...
class RequestLogin {
    ...
    async execute(context, params) {
        ...
        const request_user_help = async () => {
            await context.lyvoConfig.chromeProxy.tabs.sendMessage(tabId, {
        ...
        try {
            return await this.awaitLogin(context.lyvoConfig.chromeProxy, tabId, task_id);
        }
        finally {
            clearInterval(login_interval);
        }
    ...
    async isLoginIn(context) {
        let windowId = await getWindowId(context);
        let screenshot_result = await screenshot(context.lyvoConfig.chromeProxy, windowId, true);
        let messages = [
            {
                role: 'user',
                content: [
...
class SwitchTab {
    ...
    async execute(context, params) {
        ...
        let tab = await context.lyvoConfig.chromeProxy.tabs.update(tabId, { active: true });
        context.variables.set('tabId', tab.id);
        context.variables.set('windowId', tab.windowId);
