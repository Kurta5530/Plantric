  ...
  if (!tabId) {
    ...
    if (windowId) {
      try {
        ...
        logger.debug("getCurrentTabId(context.lyvoConfig.chromeProxy, windowId) returns " + tabId);
      } catch (e) {
        tabId = await getCurrentTabId(context.lyvoConfig.chromeProxy);
        logger.debug("getCurrentTabId(context.lyvoConfig.chromeProxy, windowId) throws an error");
        logger.debug("getCurrentTabId(context.lyvoConfig.chromeProxy) returns " + tabId);
        context.variables.delete('windowId');
      }
    } else {
      tabId = await getCurrentTabId(context.lyvoConfig.chromeProxy);
      logger.debug("getCurrentTabId(context.lyvoConfig.chromeProxy) #2 returns " + tabId);
    }
