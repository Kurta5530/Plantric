...
* \`new_tab [url]\`: Open a new tab window and open the URL, eg: \`new_tab https://www.google.com\``,
  ...
  async execute(
    ...
    if (command == 'tab_all') {
      result = [];
      let tabs = await context.lyvoConfig.chromeProxy.tabs.query({ windowId: windowId });
      ...
      if (!tab.active) {
        await context.lyvoConfig.chromeProxy.tabs.update(tab.id as number, { active: true });
      }
      let newTabId = tab.id;
      ...
      let tab = await waitForTabComplete(context.lyvoConfig.chromeProxy, tabId);
      let tabInfo: TabInfo = { tabId, windowId: tab.windowId, title: tab.title, url: tab.url };
      result = tabInfo;
      ...
      if (newWindow) {
        ...
        let windowId = await getWindowId(context);
        tab = await open_new_tab(context.lyvoConfig.chromeProxy, url, false, windowId);
        context.callback?.hooks?.onTabCreated?.(tab.id as number);
      }
