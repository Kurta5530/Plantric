...
* \`switch_tab\`: Switch to the specified tab by tabId.`,
  ...
  async execute(
    ...
    // build return value
    let tabs: chrome.tabs.Tab[] = await context.lyvoConfig.chromeProxy.tabs.query({});
    ...
    if (tabs.length > 0) {
      ...
      let currentTab: chrome.tabs.Tab = await context.lyvoConfig.chromeProxy.tabs.get(currentTabId);
